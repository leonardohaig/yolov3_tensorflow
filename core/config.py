...
__C.TRAIN.INPUT_SIZE            = [320,352,384,416] #训练时，输入图片尺寸。多尺度训练
__C.TRAIN.DATA_AUG              = False # 是否进行数据增强，data augmentation,以扩大数据集（对原始图像进行裁剪、平移等变换）
__C.TRAIN.LEARN_RATE_INIT       = 5e-4 #初始学习率
...
__C.TRAIN.MODEL_SAVE_DIR        = '/home/liheng/liheng/bdd100k_my_checkpoint' #ckpt文件保存路径,同时，开始训练时自动从该路径加载最近一次训练结果
__C.TRAIN.LOG_SAVE_DIR          = './data/log2/' # 训练日志保存路径,程序会自动删除该路径并重建，务必确保该文件夹为空
